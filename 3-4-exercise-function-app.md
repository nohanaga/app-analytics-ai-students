...
API を完成させるには、まず、スターター API コードを *api*という名前のフォルダーに移動します。 これは、Static Web Apps インスタンスを作成したときに、既定で api_location に設定されるフォルダー名です。
  ...
  1. 「**Git: Commit All**」と入力して選択します。
     > **注意**  
     > Git の `user name` と `user email` が設定されていない場合は、エラーメッセージが表示されますので、以下のコマンドで設定した上で、上記手順を再実施します。  
     > `git config --global user.name "[各自のユーザー名]"`   
     > `git config --global user.email "[各自のE-mailアドレス]"`
